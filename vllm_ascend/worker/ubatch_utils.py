...
def create_ubatch_slices(
    num_scheduled_tokens: np.ndarray, token_split_points: list[int], request_level_split: bool = False
) -> UBatchSlices:
    ...
    if not request_level_split:
        ...
        for end_token in all_points:
            req_start = int(np.searchsorted(cu_num_tokens, start_token, side="right") - 1)
            req_stop = int(np.searchsorted(cu_num_tokens, end_token, side="right") - 1)
            req_slice = slice(req_start, req_stop)
            token_slice = slice(cu_num_tokens[req_start], cu_num_tokens[req_stop])
            ubatch_slices.append(UBatchSlice(req_slice, token_slice))
            start_token = end_token
...
def maybe_create_ubatch_slices(
    ...
    ubatch_slices = create_ubatch_slices(num_scheduled_tokens_per_request, token_split_points, request_level_split)
...
def create_core_control_context(aic_core: int, aiv_core: int):
    ...
    return NPUCoreControlContextManager(
        comm_aiv_core=comm_aiv_core, comm_aic_core=comm_aic_core, current_stream=current_stream
    )
