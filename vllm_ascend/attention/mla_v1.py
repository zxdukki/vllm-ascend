...
class AscendMLAMetadataBuilder(MLACommonMetadataBuilder[AscendMLAMetadata]):
    ...
    def pad_actual_seq_len_q_mtp_enable_pad(self, num_reqs_pad_size, num_reqs,
        ...
        need_padding = (
            ...
            actual_seq_lengths_q[-1] > FIA_SEQ_LEN_LIMIT)
        if need_padding:
    ...
    def build_chunked_metadata(
        ...
        chunk_starts = (torch.arange(
            self.num_chunks, dtype=torch.int32).unsqueeze(1).expand(
                -1, self.num_prefills) * self.max_context_chunk)
...
class AscendMLAImpl(MLAAttentionImpl):
        ...
        with torch.npu.stream(update_stream):
            for key, param, handle, event in zip(
                ...
                if speculative_config and speculative_config.method == "mtp" and not forward_context.is_draft_model:
                    ...
                    seq_lens_list = seq_lens_list + [0] * (num_tokens -
                                                           len(seq_lens_list))
    ...
    def _q_proj_and_k_up_proj(self, x):
        q_nope, q_pe = (self.q_proj(x)[0].view(
            -1, self.num_heads, self.qk_head_dim).split(
    ...
    def _process_weights_for_fused_mlapo(self, act_dtype: torch.dtype):
        kv_a_proj_wt = self.fused_qkv_a_proj.weight.data[
            ..., self.q_lora_rank:].contiguous()  # type: ignore[union-attr]
        ...
        self.beta1 = torch.zeros_like(self.gamma1) if (
            _bias := self.q_a_layernorm.bias
        ) is None else _bias.data  # type: ignore[union-attr]
    ...
    def exec_kv_decode(
        ...
        k_pe, k_nope, _, _ = torch_npu.npu_kv_rmsnorm_rope_cache(
            ...
            variance_epsilon,  # type: ignore[union-attr]
    ...
    def mla_preprocess_prefill(self, q_c, kv_no_split, kv_cache,
        ...
        prefill_q = self.q_proj(prefill_q_c)[0].view(-1, self.num_heads,
                                                     self.qk_head_dim)
        ...
        prefill_k_nope, prefill_value = (
            self.kv_b_proj(prefill_k_c_normed)[0].view(
                -1, self.num_heads,
